...
//!
//! Реализован минимум, нужный играм вроде Plants vs Zombies, которые строят
//! граф из RemoteIO + 3D Mixer и устанавливают input-render callbacks через
//! `AUGraphSetNodeInputCallback`. Пуллинг через граф мы не моделируем: каждая
//! input-шина 3D Mixer-а превращается в отдельный OpenAL-источник, callback
//! дёргается из общего run-loop'а через `audio_unit::render_audio_unit`.
...
struct GraphState {
    ...
    is_initialized: bool,
    is_running: bool,
    /// Какой узел является конечным выходом (RemoteIO). Нужен, чтобы знать,
    /// какой `AudioUnit` стартовать в `AUGraphStart`.
...
fn AUGraphOpen(env: &mut Environment, graph: AUGraph) -> OSStatus {
    ...
    for node_id in node_ids {
        ...
        log!(
            "AUGraphOpen: node {} -> AudioUnit instance {:?}",
            node_id, guest_instance
        );
    }
...
fn AUGraphInitialize(env: &mut Environment, graph: AUGraph) -> OSStatus {
    ...
    log!("AUGraphInitialize({:?}) {} unit(s)", graph, units.len());
...
fn AUGraphStart(env: &mut Environment, graph: AUGraph) -> OSStatus {
    ...
    // Запускаем рендеринг каждого юнита, у которого есть какой-нибудь
    // render callback (либо прямой, либо bus-овой).
    for unit in units {
...
fn AUGraphStop(env: &mut Environment, graph: AUGraph) -> OSStatus {
    ...
    log!("AUGraphStop({:?}) {} unit(s)", graph, units.len());
...
fn AUGraphConnectNodeInput(
    ...
    // Соединение принято к сведению, но мы не пуллим через граф —
    // каждый узел рендерит сам себя в run-loop'е.
    let _ = (graph, src_node, src_output_number, dest_node, dest_input_number);
    0
}
