...
fn CFURLCopyNetLocation(env: &mut Environment, url: CFURLRef) -> CFStringRef {
    ...
    if port.is_null() {
        ...
        let net_location: id = msg_class![env; NSString stringWithFormat:(get_static_str(env, "%@:%@")) host port_str];
...
fn CFURLGetBytes(
    ...
    if !buffer.is_null() && buffer_length > 0 {
        ...
        env.mem.write(buffer + copy_length, 0); // Null terminate
    }
...
fn CFURLCreateStringByAddingPercentEscapes(
    ...
    // Convert encoding
    let _ = encoding; // TODO: Use encoding for non-UTF8
