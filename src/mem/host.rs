...
pub(super) unsafe fn allocate_memory(size: usize) -> std::io::Result<*mut core::ffi::c_void> {
    ...
    let ptr = unsafe {
        VirtualAlloc(std::ptr::null(), size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE)
    };
    if ptr.is_null() { return Err(std::io::Error::last_os_error()); }
    Ok(ptr)
}
...
pub(super) unsafe fn allocate_memory(size: usize) -> std::io::Result<*mut core::ffi::c_void> {
    ...
    assert!(host_page_size >= PAGE_SIZE, "Hosts with smaller than 4KiB pages are not supported.");
    
    let ptr = unsafe {
        mmap(std::ptr::null_mut(), size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)
    };
...
#[cfg(windows)]
pub(super) unsafe fn free_memory(address: *mut core::ffi::c_void, _size: usize) -> std::io::Result<()> {
...
#[cfg(unix)]
pub(super) unsafe fn free_memory(address: *mut core::ffi::c_void, size: usize) -> std::io::Result<()> {
    use libc::munmap;
    let res = unsafe { munmap(address, size) };
    if res == -1 { return Err(std::io::Error::last_os_error()); }
    Ok(())
}
